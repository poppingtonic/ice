...
class PlaceboDescriptionAnthropic(PlaceboDescription):
    name = "PlaceboDescriptionAnthropic"
    agent_str = "anthropic-junior-h-helpful-v7-s750"

    def make_prompt(self, paper: Paper, experiment: str) -> str:
        paper_text = truncate_by_tokens(str(paper), max_tokens=7000)
        return f"""
{HUMAN_PROMPT} What is a placebo?
{AI_PROMPT} A placebo is a treatment which is specifically designed to be indistinguishable from the real treatment but has no real effect.
...
{HUMAN_PROMPT} In this paper, what was the placebo in the "{experiment}" experiment?
